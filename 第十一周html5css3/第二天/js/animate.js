...
(function () {
    ...
    function  animate(opt) {
        ...
        var effect=defaultOpt.effect;//默认的运动形式
        ...
        timer=setInterval(function () {
            ...
            if(time>=duration){
                utils.css(el,target);
                clearInterval(timer);
                defaultOpt.callback && defaultOpt.callback.call(el);
                //fire.call(el,'myAnimate');
                return;
            }
            //求出每个时间点的最新位置；
            for(var attr in change){
                var curPos=effect(time,begin[attr],change[attr],duration);
                utils.css(el,attr,curPos);
            }
        },10)
