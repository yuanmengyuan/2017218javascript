...
(function(){
    ...
    function animate(opt){
        opt=opt||{};//为了避免错误
        if(!opt.ele) return;//如果传参错误的话，后面代码就不执行了
        //默认的
        var defaultOpt={
            duration:1000,
            effect:'Linear'
        ...
        var tmpEffect=defaultOpt.effect;//默认给的是匀速运动
        ...
        if(typeof tmpEffect==='number'){
            ary=ary[tmpEffect%ary.length].split('-');
            tmpEffect=ary.length>=2?ymyEffect[ary[0]][ary[1]]:ymyEffect[ary[0]];
        }else if(typeof tmpEffect==='object'){
            tmpEffect=ary.length>=2?ymyEffect[tmpEffect[0]][tmpEffect[1]]:ymyEffect[tmpEffect[0]];
        }
        //下面都是为参数做准备的
        var begin={},change={};
        //遍历target来求出begin和change的值；
